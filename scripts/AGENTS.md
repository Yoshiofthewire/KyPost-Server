...
- `bootstrap.sh` must keep running before the `chown -R llamalab:llamalab /llama_lab` step in `entrypoint.sh` — it writes as root, and that chown is what hands the resulting files (`admin.env` included) to `llamalab`
- `pull-ollama-model.sh` is idempotent; safe to re-run

## Verification

- On a fresh install `bootstrap.sh` must produce a valid `admin.env` with a non-empty scrypt hash owned by `llamalab`; on an install with `users.json` or `admin.env` present it must leave both untouched
