...
const KEY_INDEX_PREFIX = "keyid:";
const IP_INDEX_PREFIX = "ipkey:";
 ...
 * true when allowed. Fails open if the binding is missing or errors.
 */
...
/**
 * Record one accepted send to Analytics Engine (off the KV write path). Best-effort.
 */
...
/**
 * Mint an API key, persist only its hash, and return the record plus the raw key.
 */
...
/**
 * Delete a key and all its indexes by id. Also clears the per-IP index.
 */
